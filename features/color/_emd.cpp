/*
    ...
    Python module adapter added.
    
    Felipe Costa, felipealco@dcc.ufmg.br
    Luam Totti, luamct@dcc.ufmg.br
*/
...
typedef struct { double X,Y,Z; } feature_t;
/*****************************************************************************/
...
float process(int n_bins, pyublas::numpy_vector<double> bins_h_, pyublas::numpy_vector<double> bins_, 
							 pyublas::numpy_vector<double> center_)
{
	signature_t s1, s2;
	feature_t *center;
	double *bins_h, *bins;
	
	center = reinterpret_cast<feature_t*>(center_.data().data());
	bins = reinterpret_cast<double*>(bins_.data().data());
	bins_h = reinterpret_cast<double*>(bins_h_.data().data());
